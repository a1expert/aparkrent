...
//        return var_dump(\Yii::getAlias('@frontend/web') . '/images/files/image_thumbs/');die;
    ...
    {
        self::createDirectory(\Yii::getAlias('@frontend/web') . '/images/files/image_thumbs/');
//        self::createDirectory(\Yii::getAlias('@app') . '/web/assets/image_thumbs/');
        $no_photo_image_path = \Yii::getAlias('@common') . '/files/noimage.png';
        $preview_url = '/images/files/image_thumbs/' . $width . 'x' . $height . '.' . self::getExtensionByFileName($no_photo_image_path);
        $preview_url_path = \Yii::getAlias('@frontend/web') . $preview_url;
    ...
    public static function generateImageWithChars($text, $width, $height)
    {
        list($char1, $char2) = StringHelper::getFirstCharsOfWords($text);
        $charFiles = [
            'а' => '1.png',
            'б' => '2.png',
            'в' => '3.png',
        ...
        try {
            // Сначала создаем наше изображение штампа вручную с помощью GD
            $background = imagecreatefrompng(\Yii::getAlias('@common') . '/files/circle_background.png');
            imagesavealpha($background, true);

            if (!isset($charFiles[$char1])) {
                return self::getNoPhotoImage($width, $height);
            }
            $char1Path = \Yii::getAlias('@common') . '/files/chars/' . $charFiles[$char1];
            $char1Image = imagecreatefrompng($char1Path);
            list($char1Width, $char1Height) = getimagesize($char1Path);
            $firstCharDesX = 90 - $char1Width;
            $firstCharDesY = 50;

            if (isset($charFiles[$char2])) {
                $char2Path = \Yii::getAlias('@common') . '/files/chars/' . $charFiles[$char2];
                $char2Image = imagecreatefrompng($char2Path);
                list($char2Width, $char2Height) = getimagesize($char2Path);
                imagecopy($background, $char2Image, 110, 50, 0, 0, $char2Width, $char2Height);
            } else {
                //Переназначить координаты
                $firstCharDesX = 100 - $char1Width / 2;
            }
            // Слияние букв с фотографией
            imagecopy($background, $char1Image, $firstCharDesX, $firstCharDesY, 0, 0, $char1Width, $char1Height);

            $newPath = self::generateFilePath(\Yii::getAlias('@webroot'), '/assets/avatars/', 'png');
            imagepng($background, \Yii::getAlias('@webroot') . $newPath);
            return $newPath;
        } catch (\Exception $e) {
            \Yii::error('Ошибка добавлении водяного знака: ' . $e->getMessage());
            echo 'Ошибка добавлении водяного знака: ' . $e->getMessage();
            return 'Ошибка добавлении водяного знака: ' . $e->getMessage();
        }
    }
